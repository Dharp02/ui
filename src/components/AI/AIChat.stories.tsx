...
- **No live region.** The thread is a plain scrolling \`div\`; new or streamed assistant messages, the typing indicator and the header's "Generating..." text are not announced. \`onSendMessage\` receives trimmed text only — the composer clears optimistically (Enter sends, Shift+Enter newlines, so focus stays in the textarea); if \`onSendMessage\` throws or rejects, the draft is restored and the failure is reported through \`composerProps.onError\` ("Failed to send message").
