...
- RTL: outgoing bubbles use \`flex-row-reverse\`/\`justify-end\`, the reply preview has a physical \`border-l-4\`, and the standalone \`MessageComposer\`'s trailing slot is \`right-1\` (\`MessageThread\`'s \`ChatComposer\` uses logical properties). Theming is hard-coded \`neutral-*\`/\`white\`/\`primary-*\` utilities. Depends on \`class-variance-authority\`. Entry \`@mieweb/ui\`.`,
      },
    },
    catalog: {
      entry: '@mieweb/ui',
      relationships: [
        ...
        },
        {
          ...
          why: 'Both composers share the same @mention autocomplete module and reply-to contract (replyTo / onCancelReply); ChatComposer is the standardized input (+ menu, mic, stop, agent/model selectors) that MessageThread now mounts, while MessageComposer remains for standalone use in existing surfaces.',
        },
      ],
    },
  },
